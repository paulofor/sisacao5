...
public class NotebookTreinoTensorFlow extends NotebookObj {
	...
	protected void montaScript(JSONArray notebook) {
		...
		montaEntrada1(notebook);
		if (this.treinoRede.getTipoExemploTreino2Id()!=0) {
			montaEntrada2(notebook);
		}
	...
	private void montaEntrada1(JSONArray notebook) {
		CelulaNotebook celula = new CelulaNotebook();
		celula.setTitulo("Dados Treino " + this.treinoRede.getRedeNeural().getNome() + " (entrada 1) " , 1);
		...
		celula.adicionaLinha("url = 'http://vps-40d69db1.vps.ovh.ca:23004/api/ExemploTreinoAcaoSaidas/listaParaTreinoEntradaSaida?diaNumInicio={}&diaNumFinal={}&idGrupoAcao={}&idRegraProjecao={}&idTipoExemplo={}'.format(diaInicio,diaFinal,idGrupo,idRegra,idTipo)");;
		...
		celula.adicionaLinha("df.head()");
		insereCelula(notebook,celula);
		
		
		celula = new CelulaNotebook();
		celula.adicionaLinha("dfTratadoX = df['campoX'].str.split(',').apply(lambda x: [float(val) for val in x])");
		...
		celula.adicionaLinha("    print(\"Entrada1 array contém valores não numéricos.\")");
		celula.adicionaLinha("else:");
		celula.adicionaLinha("    print(\"Entrada1 contém apenas valores numéricos.\")");
		insereCelula(notebook,celula);
		...
		celula.adicionaLinha("if has_non_numeric:");
		celula.adicionaLinha("    print(\"Saida1 contém valores não numéricos.\")");
		celula.adicionaLinha("else:");
		celula.adicionaLinha("    print(\"Saida1 contém apenas valores numéricos.\")");
		insereCelula(notebook,celula);
	}
	private void montaEntrada2(JSONArray notebook) {
		CelulaNotebook celula = new CelulaNotebook();
		celula.setTitulo("Dados Treino " + this.treinoRede.getRedeNeural().getNome() + " (entrada 2) " , 1);
		celula.adicionaLinha("diaInicio = '" + this.treinoRede.getPeriodoTreinoRede().getDiaNumInicioTreino() + "'");
		celula.adicionaLinha("diaFinal = '" + this.treinoRede.getPeriodoTreinoRede().getDiaNumFinalTreino() + "'");
		celula.adicionaLinha("idGrupo = '" + this.treinoRede.getGrupoAcaoId() + "'");
		celula.adicionaLinha("idRegra = '" + this.treinoRede.getRegraProjecaoId() + "'");
		celula.adicionaLinha("idTipo = '" + this.treinoRede.getTipoExemploTreino2Id() + "'");
		celula.adicionaLinha("url = 'http://vps-40d69db1.vps.ovh.ca:23004/api/ExemploTreinoAcaoSaidas/listaParaTreinoEntradaSaida?diaNumInicio={}&diaNumFinal={}&idGrupoAcao={}&idRegraProjecao={}&idTipoExemplo={}'.format(diaInicio,diaFinal,idGrupo,idRegra,idTipo)");;
		insereCelula(notebook,celula);
		...
		celula.adicionaLinha("if has_non_numeric:");
		celula.adicionaLinha("    print(\"Entrada2 array contém valores não numéricos.\")");
		celula.adicionaLinha("else:");
		celula.adicionaLinha("    print(\"Entrada2 contém apenas valores numéricos.\")");
		insereCelula(notebook,celula);
		...
		celula.adicionaLinha("has_non_numeric = np.issubdtype(Saida1.dtype, np.number) is False");
		celula.adicionaLinha("if has_non_numeric:");
		celula.adicionaLinha("    print(\"Saida2 contém valores não numéricos.\")");
		celula.adicionaLinha("else:");
		celula.adicionaLinha("    print(\"Saida2 contém apenas valores numéricos.\")");
		insereCelula(notebook,celula);
	}
