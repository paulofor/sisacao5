package gerador.insereexemplotreinoindiceentradaperiodo.passo.suporte;
...
public class DadosTreinoX {
	...
	public void calcula(List<DiaPregao> dias, double valorReferencia) {
		...
		for (int i=inicio;i<indDiaPrevisao;i++) {
			...
			for (int x=0; x<diaCorrente.getCotacaoIntradayIndiceResultados().size();x++) {
				CotacaoIntradayIndiceResultado cotacao = diaCorrente.getCotacaoIntradayIndiceResultados().get(x);
