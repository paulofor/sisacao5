...
import coletorjava.parse.dados.basico.MesAnoDadosParseBase;
import coletorjava.modelo.MesAno;
import coletorjava.log.ArquivoLog;


public abstract class MesAnoDetalheCallbackHtmlBase  extends CallbackParseHtml{

	protected MesAnoDadosParseBase dadosParse;
	...
	public final void setDados(IDadosParse paramIDadosParse) {
		dadosParse = (MesAnoDadosParseBase) paramIDadosParse;
		ArquivoLog.getInstancia().salvaLog("MesAno(detalhe):"  + dadosParse.displayLog(dadosParse.getItemDetalhe()));
	}
	...
	public final void finalizacaoOk() {
		//if (lista==null) throw new RuntimeException("objeto lista em MesAnoListaCallbackHtmlBase esta null");
		finalizaObjeto();
	...
	private void limpaListasInternas() {
		...
		this.dadosParse.getItemDetalhe().setListaResultadoPrevisao_Gera(null);
 	
		this.dadosParse.getItemDetalhe().setListaResultadoMes_BasePara(null);
