...
import coletorjava.parse.dados.basico.ResultadoMesDadosParseBase;
import coletorjava.modelo.ResultadoMes;
import coletorjava.log.ArquivoLog;


public abstract class ResultadoMesDetalheCallbackHtmlBase  extends CallbackParseHtml{

	protected ResultadoMesDadosParseBase dadosParse;
	...
	public final void setDados(IDadosParse paramIDadosParse) {
		dadosParse = (ResultadoMesDadosParseBase) paramIDadosParse;
		ArquivoLog.getInstancia().salvaLog("ResultadoMes(detalhe):"  + dadosParse.displayLog(dadosParse.getItemDetalhe()));
	}
	...
	public final void finalizacaoOk() {
		//if (lista==null) throw new RuntimeException("objeto lista em ResultadoMesListaCallbackHtmlBase esta null");
		finalizaObjeto();
	...
	private void limpaListasInternas() {
		...
		this.dadosParse.getItemDetalhe().setListaResultadoPrevisao_Semelhante(null);
