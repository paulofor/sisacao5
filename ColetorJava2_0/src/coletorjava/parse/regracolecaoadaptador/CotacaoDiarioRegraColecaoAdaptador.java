...
import coletorjava.parse.callback.CotacaoDiarioListaCallbackHtml;
import coletorjava.parse.callback.CotacaoDiarioDetalheCallbackHtml;
import coletorjava.parse.dados.CotacaoDiarioDadosParse;
...
public abstract class CotacaoDiarioRegraColecaoAdaptador  {
	...
	protected CotacaoDiarioDadosParse dadosParse = null;
	
	public CotacaoDiarioRegraColecaoAdaptador() {
		exec = new ExecutadorParse();
		dadosParse = new CotacaoDiarioDadosParse();
	}
	...
	protected ICallbackParse criaListaCallbackHtml() {
		return new CotacaoDiarioListaCallbackHtml();
	}
	protected ICallbackParse criaDetalheCallbackHtml() {
		return new CotacaoDiarioDetalheCallbackHtml();
	}
	...
	protected void preparaDadosLista(CotacaoDiarioDadosParse dados) {
	}
	...
	public void atualizaDetalhe(DaoConexao conexao) throws DaoException {
		CotacaoDiarioDao daoFonte = DBB.getInstancia().getCotacaoDiarioDao(); // 
		daoFonte.setConexao(conexao);
		List<CotacaoDiario> lista = daoFonte.ListaCorrente();
		Iterator<CotacaoDiario> itLista = lista.iterator();
		while (itLista.hasNext()) {
			CotacaoDiario corrente = itLista.next();
	...
	public final void atualizaDetalhe(CotacaoDiario itemRaiz, DaoConexao conexao) throws DaoException {
	...
	protected void preparaDadosDetalhe(CotacaoDiarioDadosParse dados) {
	}
	public CotacaoDiario getItemRaiz() {
	...
	private void atualizaItens(DaoConexao conexao) throws DaoException {
		...
		ICallbackParse callback = new CotacaoDiarioListaCallbackHtml();
		CotacaoDiarioDadosParse dadosParse = new CotacaoDiarioDadosParse();
		dadosParse.setConexao(conexao);
