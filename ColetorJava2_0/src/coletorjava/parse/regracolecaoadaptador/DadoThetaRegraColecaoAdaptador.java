...
import coletorjava.parse.callback.DadoThetaListaCallbackHtml;
import coletorjava.parse.callback.DadoThetaDetalheCallbackHtml;
import coletorjava.parse.dados.DadoThetaDadosParse;
...
public abstract class DadoThetaRegraColecaoAdaptador  {
	...
	protected DadoThetaDadosParse dadosParse = null;
	
	public DadoThetaRegraColecaoAdaptador() {
		exec = new ExecutadorParse();
		dadosParse = new DadoThetaDadosParse();
	}
	...
	protected ICallbackParse criaListaCallbackHtml() {
		return new DadoThetaListaCallbackHtml();
	}
	protected ICallbackParse criaDetalheCallbackHtml() {
		return new DadoThetaDetalheCallbackHtml();
	}
	...
	protected void preparaDadosLista(DadoThetaDadosParse dados) {
	}
	...
	public void atualizaDetalhe(DaoConexao conexao) throws DaoException {
		DadoThetaDao daoFonte = DBB.getInstancia().getDadoThetaDao(); // 
		daoFonte.setConexao(conexao);
		List<DadoTheta> lista = daoFonte.ListaCorrente();
		Iterator<DadoTheta> itLista = lista.iterator();
		while (itLista.hasNext()) {
			DadoTheta corrente = itLista.next();
	...
	public final void atualizaDetalhe(DadoTheta itemRaiz, DaoConexao conexao) throws DaoException {
	...
	protected void preparaDadosDetalhe(DadoThetaDadosParse dados) {
	}
	public DadoTheta getItemRaiz() {
	...
	private void atualizaItens(DaoConexao conexao) throws DaoException {
		...
		ICallbackParse callback = new DadoThetaListaCallbackHtml();
		DadoThetaDadosParse dadosParse = new DadoThetaDadosParse();
		dadosParse.setConexao(conexao);
