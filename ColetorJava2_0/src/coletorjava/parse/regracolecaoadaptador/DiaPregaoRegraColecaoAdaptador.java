...
import coletorjava.parse.callback.DiaPregaoListaCallbackHtml;
import coletorjava.parse.callback.DiaPregaoDetalheCallbackHtml;
import coletorjava.parse.dados.DiaPregaoDadosParse;
...
public abstract class DiaPregaoRegraColecaoAdaptador  {
	...
	protected DiaPregaoDadosParse dadosParse = null;
	
	public DiaPregaoRegraColecaoAdaptador() {
		exec = new ExecutadorParse();
		dadosParse = new DiaPregaoDadosParse();
	}
	...
	protected ICallbackParse criaListaCallbackHtml() {
		return new DiaPregaoListaCallbackHtml();
	}
	protected ICallbackParse criaDetalheCallbackHtml() {
		return new DiaPregaoDetalheCallbackHtml();
	}
	...
	protected void preparaDadosLista(DiaPregaoDadosParse dados) {
	}
	...
	public void atualizaDetalhe(DaoConexao conexao) throws DaoException {
		DiaPregaoDao daoFonte = DBB.getInstancia().getDiaPregaoDao(); // 
		daoFonte.setConexao(conexao);
		List<DiaPregao> lista = daoFonte.ListaCorrente();
		Iterator<DiaPregao> itLista = lista.iterator();
		while (itLista.hasNext()) {
			DiaPregao corrente = itLista.next();
	...
	public final void atualizaDetalhe(DiaPregao itemRaiz, DaoConexao conexao) throws DaoException {
	...
	protected void preparaDadosDetalhe(DiaPregaoDadosParse dados) {
	}
	public DiaPregao getItemRaiz() {
	...
	private void atualizaItens(DaoConexao conexao) throws DaoException {
		...
		ICallbackParse callback = new DiaPregaoListaCallbackHtml();
		DiaPregaoDadosParse dadosParse = new DiaPregaoDadosParse();
		dadosParse.setConexao(conexao);
