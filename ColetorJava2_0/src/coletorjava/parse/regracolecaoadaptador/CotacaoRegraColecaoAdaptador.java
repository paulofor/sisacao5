...
import coletorjava.parse.callback.CotacaoListaCallbackHtml;
import coletorjava.parse.callback.CotacaoDetalheCallbackHtml;
import coletorjava.parse.dados.CotacaoDadosParse;
...
public abstract class CotacaoRegraColecaoAdaptador  {
	...
	protected CotacaoDadosParse dadosParse = null;
	
	public CotacaoRegraColecaoAdaptador() {
		exec = new ExecutadorParse();
		dadosParse = new CotacaoDadosParse();
	}
	...
	protected ICallbackParse criaListaCallbackHtml() {
		return new CotacaoListaCallbackHtml();
	}
	protected ICallbackParse criaDetalheCallbackHtml() {
		return new CotacaoDetalheCallbackHtml();
	}
	...
	protected void preparaDadosLista(CotacaoDadosParse dados) {
	}
	...
	public void atualizaDetalhe(DaoConexao conexao) throws DaoException {
		CotacaoDao daoFonte = DBB.getInstancia().getCotacaoDao(); // 
		daoFonte.setConexao(conexao);
		List<Cotacao> lista = daoFonte.ListaCorrente();
		Iterator<Cotacao> itLista = lista.iterator();
		while (itLista.hasNext()) {
			Cotacao corrente = itLista.next();
	...
	public final void atualizaDetalhe(Cotacao itemRaiz, DaoConexao conexao) throws DaoException {
	...
	protected void preparaDadosDetalhe(CotacaoDadosParse dados) {
	}
	public Cotacao getItemRaiz() {
	...
	private void atualizaItens(DaoConexao conexao) throws DaoException {
		...
		ICallbackParse callback = new CotacaoListaCallbackHtml();
		CotacaoDadosParse dadosParse = new CotacaoDadosParse();
		dadosParse.setConexao(conexao);
