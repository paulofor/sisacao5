...
import coletorjava.parse.callback.TickerListaCallbackHtml;
import coletorjava.parse.callback.TickerDetalheCallbackHtml;
import coletorjava.parse.dados.TickerDadosParse;
...
public abstract class TickerRegraColecaoAdaptador  {
	...
	protected TickerDadosParse dadosParse = null;
	
	public TickerRegraColecaoAdaptador() {
		exec = new ExecutadorParse();
		dadosParse = new TickerDadosParse();
	}
	...
	protected ICallbackParse criaListaCallbackHtml() {
		return new TickerListaCallbackHtml();
	}
	protected ICallbackParse criaDetalheCallbackHtml() {
		return new TickerDetalheCallbackHtml();
	}
	...
	protected void preparaDadosLista(TickerDadosParse dados) {
	}
	...
	public void atualizaDetalhe(DaoConexao conexao) throws DaoException {
		TickerDao daoFonte = DBB.getInstancia().getTickerDao(); // 
		daoFonte.setConexao(conexao);
		List<Ticker> lista = daoFonte.ListaCorrente();
		Iterator<Ticker> itLista = lista.iterator();
		while (itLista.hasNext()) {
			Ticker corrente = itLista.next();
	...
	public final void atualizaDetalhe(Ticker itemRaiz, DaoConexao conexao) throws DaoException {
	...
	protected void preparaDadosDetalhe(TickerDadosParse dados) {
	}
	public Ticker getItemRaiz() {
	...
	private void atualizaItens(DaoConexao conexao) throws DaoException {
		...
		ICallbackParse callback = new TickerListaCallbackHtml();
		TickerDadosParse dadosParse = new TickerDadosParse();
		dadosParse.setConexao(conexao);
